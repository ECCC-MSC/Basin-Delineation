...
#'
#' @param NTS character NTS string in format DDDLDD or DDDL (d=digit, L=letter) e.g 075M05 or 014K
#' @param product character string specifying which data product to get (one of 'CDEM', 'CDED', 'CDSM')
#' @return character path to CDED DEM tile for selected NTS sheet
#' @export
#' @keywords internal
...
#' Download DEM Sheet
#' @param NTS character string, name of NTS tile (e.g. '075C' or '014D03') or USGS NED tile. If specified as
...
#' @param DEM.dir character string, path to directory where DEM tiles are stored. This function creates a nested
...
#' in the DEM.dir directory
#' @param product character string specifying which data product to get (one of 'CDEM', 'CDED', 'CDSM', or 'NED')
...
DownloadSingleDEM <-  function(DEM.ID, DEM.dir, replace=F, product='CDED'){
  ...
  dir.create(file.path(DEM.dir, paste(file.paths[1:2], collapse='/')), showWarnings = F, recursive=T)
...
#' Download and Unzip DEM
#' @description a subroutine of \link{DownloadSingleDEM}
...
#' Download Multiple DEM files
#'
#' @description a wrapper for \link{DownloadSingleDEM} that allows a vector of NTS sheets
#' or USGS DEM tile names to be specified
#' @param DEM character string, name of DEM tile (e.g. '075C' or '014D03'). If specified as
#' a 250k tile (4 characters), a 250k tile will be downloaded. If specified as a 50k tile (6 character)
#' then a 50k tile will be downloaded.
#' @param DEM.dir character string, path to directory where DEM tiles are stored. This function creates a nested
#' file hierarchy that matches that of the FTP site.
...
DownloadMultipleDEM <- function(DEM, DEM.dir, force.50k=F, product='CDED'){
    if (toupper(product) %in% c('CDED', 'CDEM', 'CDSM') & !all(grepl("^\\d{3}[[:alpha:]](\\d{2})?$", DEM))){
      stop("Bad format for one or more NTS strings")
    }
    if (force.50k & product %in% c('CDED', 'CDSM')){
      DEM <- unlist(lapply(DEM, function(x) paste(x, sprintf("%02d", seq(1,16)), sep='')))
    }
    files <- t(sapply(DEM, DownloadSingleDEM, DEM.dir=DEM.dir, replace=F, product=product))
...
#' Create DEM mosaic
#'
#' @description Downloads a set of DEM tiles and moasaicks them into one SAGA grid file.
#' @param DEM character string, name of NTS tile (e.g. '075C' or '014D03') or USGS NED 1arc-sec tile
#' (e.g. 'n52e104'). If specified as
#' a 250k NTS tile (4 characters), a 250k NTS tile will be downloaded. If specified as a 50k NTS tile (6 character)
#' then a 50k NTS tile will be downloaded.
#' @param DEM.dir character string, path to directory where DEM tiles are stored. This function creates a nested
#' file hierarchy that matches that of the FTP site.
#' @param output.dir character string specifying where output file should be saved
#' @param force.50k logical, returns 50k tiles instead of 250k tiles even if the tiles are specified as 250k tiles.
#' e.g. using "075C" would use 075C01, 075C02, 075C03 and so on. Ignored if using 'NED' as data product.
#' @param product character, which DEM product to use.  One of ('CDED', 'CDEM', 'NED')
...
CompileDEM <- function(DEM, DEM.dir, output.dir, force.50k=F, product='CDED'){
  files <- DownloadMultipleDEM(DEM=DEM, DEM.dir=DEM.dir, force.50k=force.50k, product=product)
...
#' Create DEM mosaic to cover spatial object
#'
#' @description Covers a shape with a DEM grid
#' @param geom1 An R Spatial* object to be 'buffered' with the DEM
#' @param tileindex spatialpointspolygon of DEM tile indices or path to shapefile of tile index
...
#' @details Builds a DEM from elevation dataset that either extends a fixed distance on either
#' side of a spatialobject (if a tol(erace) is specified) or intersects the DEM index (if
...
OverlayDEM <- function(geom1, tileindex, DEM.dir, output.dir, tol,  product='CDED', tile.id.field="NTS_SNRC", ...){
  ...
  # build and mosaic
  grid <- CompileDEM(DEM=tile.names, DEM.dir=DEM.dir, output.dir=output.dir, product=product, ...)
